...
#
# Generated on 2025-02-10T19:04:00Z by kres 5e9dc91.
...
EXTENSIONS_REF ?= 8a17f71183c1b6f5347f7fdadfee077209ede48e
...
all: unit-tests talos-metal-agent image-talos-metal-agent image-boot-assets push-boot-assets lint
...
.PHONY: $(ARTIFACTS)/talos-metal-agent-linux-amd64
$(ARTIFACTS)/talos-metal-agent-linux-amd64:
	@$(MAKE) local-talos-metal-agent-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: talos-metal-agent-linux-amd64
talos-metal-agent-linux-amd64: $(ARTIFACTS)/talos-metal-agent-linux-amd64  ## Builds executable for talos-metal-agent-linux-amd64.

.PHONY: $(ARTIFACTS)/talos-metal-agent-linux-arm64
$(ARTIFACTS)/talos-metal-agent-linux-arm64:
	@$(MAKE) local-talos-metal-agent-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: talos-metal-agent-linux-arm64
talos-metal-agent-linux-arm64: $(ARTIFACTS)/talos-metal-agent-linux-arm64  ## Builds executable for talos-metal-agent-linux-arm64.

.PHONY: talos-metal-agent
talos-metal-agent: talos-metal-agent-linux-amd64 talos-metal-agent-linux-arm64  ## Builds executables for talos-metal-agent.
...
push-boot-assets:
	IMAGER_TAG=$(IMAGER_TAG) PUSH_TAG=$(PUSH_TAG) BUILD_TAG=$(TAG) OUTPUT_REGISTRY_AND_USERNAME=$(OUTPUT_REGISTRY_AND_USERNAME) hack/boot-assets/push.sh
